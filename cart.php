        ...
        <?php else: ?>
            <form method="POST">
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <!-- Cart Items -->
                    <div class="checkout-card lg:col-span-2 p-8 bg-white rounded-lg shadow-sm h-fit">
                            ...
                            <table class="w-full min-w-[600px] border-collapse">
                                ...
                                <tbody>
                                    ...
                                    <tr class="border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors">
                                        ...
                                        <td class="py-6 px-4 text-accent font-bold">₹<?= number_format($item['price'], 2) ?></td>
                                        <td class="py-6 px-4">
                                        ...
                                        <td class="py-6 px-4 text-right text-accent font-black text-lg">₹<?= number_format($item['price'] * $item['quantity'], 2) ?></td>
                    ...
                    <!-- Summary -->
                    <div class="checkout-card h-fit sticky top-8 bg-white p-8 rounded-lg shadow-sm border border-gray-100">
                        <h3 class="checkout-card-title text-xl font-bold mb-6 flex items-center gap-2"><i class="fa-solid fa-receipt text-primary"></i> <?= __('order_summary') ?></h3>
                        <div class="flex justify-between items-center mb-4 text-sm font-bold text-gray-600">
                            <span><?= __('subtotal') ?></span>
                            <span>₹<?= number_format($total, 2) ?></span>
                        </div>
                        <div class="flex justify-between items-center mb-6 text-sm font-bold text-gray-600 border-b border-gray-100 pb-6">
                            <span><?= __('shipping') ?></span>
                            <span class="text-green-500"><?= __('free') ?></span>
                        </div>
                        <div class="flex justify-between items-center mb-8 text-lg font-black text-accent">
                            <span><?= __('estimated_total') ?></span>
                            <span>₹<?= number_format($total, 2) ?></span>
                        </div>
                        <a href="checkout.php" class="btn btn-primary w-full py-4 text-lg font-bold flex items-center justify-center gap-3 shadow-lg hover:shadow-xl transition-shadow">
                            <?= __('proceed_to_checkout') ?> <i class="fa-solid fa-chevron-right text-xs"></i>
                        </a>
                        <p class="text-center mt-5 text-sm text-gray-400 font-medium flex items-center justify-center gap-2">
                            <i class="fa-solid fa-shield-halved"></i> <?= __('secure_checkout') ?>
                        </p>
                    </div>
                </div>
            </form>
